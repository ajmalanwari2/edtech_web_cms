...
use App\Models\LibraryVideoBookmark;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
use App\Models\User;
use App\Models\Student;
use DataTables;
class LibraryVideoBookmarkController extends Controller
{
    /**
    ...
    public function update(Request $request)
    {
        ...
        $bookmark = LibraryVideoBookmark::where('user_id', $user_id)
                            ->where('library_video_id', $request->library_video_id )
                            ->first();
        
        if (!$bookmark) {
            // If no bookmark exists, create a new one
            $bookmark = new LibraryVideoBookmark();
            $bookmark->user_id = $user_id;
            $bookmark->library_video_id  = $request->library_video_id ;
        }
    ...
    FROM
        users AS u
        LEFT JOIN library_video_bookmarks AS b ON u.id = b.user_id 
        LEFT JOIN library_videos AS la ON la.id = b.library_video_id 
