            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header card-header-large bg-light d-flex align-items-center">
                        <div class="flex">
                            <div class="card-subtitle text-muted">List of Registered Parents</div>
                        </div>
                        <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#modal-form">Register New parent</button>
                    <button type="button" class="btn btn-info" onclick="table.ajax.reload()"
                        style="margin-left: 5px;">Reload</button>
                    </div>
                    <div class="card-body">
                        <table id="parent" class="display" style="width:100%">
                            <thead>
                                <tr>
                                    <th>Name</th>
                                    <th>Email</th>
...
<script type="text/javascript">
    ...
    var url =site_url+'api/parent/save';
    var data = {
        '_token': '{{ csrf_token() }}',
        name: $('#name').val(),
        email: $('#email').val(),
    ...
    if (!(id === undefined)) {
        url =site_url+'api/parent/update';
        data.id = id;
    }
    $.ajax({
        type: "POST",
    ...
    function loadRecord(id) {
        $.ajax({
            ...
            success: (function(data) {
                ...
                $("#saveBTN").html("Update");
                $("#modal-form-title").html("parent Update Form");
    ...
    function deleteRecord() {
        ...
        $.ajax({
            type: "POST",
            url: site_url + 'api/parent/delete',
            data: {
                id: deleteRecordID,
                '_token': '{{ csrf_token() }}'
            },
    ...
    $(document).on('hide.bs.modal', '#modal-form', function() {
        ...
        $("#saveBTN").html("Save");
        $("#modal-form-title").html("parent Save Form");
    });
